...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['cv_file']) && ($_FILES['cv_file']['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_OK) {
        ...
        $target = dirname(__DIR__) . '/uploads/cv/' . $safeName;
    ...
    if ($lettre !== '' && $cvValue !== '') {
        ...
        try {
            ...
            $insertApp = $pdo->prepare(
                "INSERT INTO application (id_user, lettre_de_motivation, cv, status, date_creation)
                 VALUES (?, ?, ?, 'pending', NOW())"
            );
            $insertApp->execute([(int)$_SESSION['user_id'], $lettre, $cvValue]);
