...
$formOpen = $compose || in_array($notice, ['invalid', 'denied', 'created', 'updated'], true) || $editOfferId > 0;
$maxOfferImageSize = 5 * 1024 * 1024;
$allowedOfferImageExt = ['jpg', 'jpeg', 'png', 'gif', 'webp'];
$allowedOfferImageMime = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
$allowedOfferStatuses = ['draft', 'published', 'paused', 'closed', 'active', 'actif', 'open', 'completed'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($action, ['create_offer', 'update_offer', 'delete_offer'], true)) {
        $postedCsrf = (string)($_POST['csrf'] ?? '');
        if (!hash_equals($offerCsrf, $postedCsrf)) {
    ...
    if ($action === 'create_offer') {
        ...
        $statusInput = trim((string)($_POST['status'] ?? 'draft'));
        $offerStatus = in_array($statusInput, $allowedOfferStatuses, true) ? $statusInput : 'draft';
        ...
        if ($titre !== '' && $description !== '' && $duree !== '' && $idProjet > 0) {
            ...
            if ($hasImageColumn && isset($_FILES['offer_image']) && $_FILES['offer_image']['error'] === UPLOAD_ERR_OK) {
                $tmpName = (string)($_FILES['offer_image']['tmp_name'] ?? '');
                $fileSize = (int)($_FILES['offer_image']['size'] ?? 0);
                $originalName = (string)($_FILES['offer_image']['name'] ?? '');
                $ext = strtolower(pathinfo($originalName, PATHINFO_EXTENSION));

                $finfo = new finfo(FILEINFO_MIME_TYPE);
                $mime = $tmpName !== '' ? (string)$finfo->file($tmpName) : '';

                if ($fileSize <= 0 || $fileSize > $maxOfferImageSize || !in_array($ext, $allowedOfferImageExt, true) || !in_array($mime, $allowedOfferImageMime, true)) {
...
if ($canManageOffers) {
    $myOffersStmt = $pdo->prepare(
        'SELECT o.id_offer, o.titre, o.budget, o.duree, ' . ($hasStatusColumn ? 'o.status' : '"" AS status') . ', ' . ($hasVerificationColumn ? 'o.verification_status' : '"not_verified" AS verification_status') . ', p.titre AS projet_titre
         FROM offre_emploi o
...
if ($verificationFilter !== '' && $hasVerificationColumn && in_array($verificationFilter, ['verified', 'not_verified'], true)) {
    $sql .= ' AND o.verification_status = ?';
    $params[] = $verificationFilter;
}

if ($statusFilter !== '' && $hasStatusColumn && in_array($statusFilter, $allowedOfferStatuses, true)) {
    $sql .= ' AND o.status = ?';
    $params[] = $statusFilter;
}
...
$sortMap = [
    'latest' => $hasCreatedAtColumn ? 'o.created_at DESC' : 'o.id_offer DESC',
    'oldest' => $hasCreatedAtColumn ? 'o.created_at ASC' : 'o.id_offer ASC',
    'budget_high' => 'o.budget DESC',
    'budget_low' => 'o.budget ASC',
    'title_asc' => 'o.titre ASC',
    'title_desc' => 'o.titre DESC',
];
$sortSql = $sortMap[$sortFilter] ?? $sortMap['latest'];
$sql .= ' ORDER BY ' . $sortSql . ' LIMIT ' . $perPage . ' OFFSET ' . $offset;
...
function normalize_offer_image_url(?string $imagePath): string
{
    ...
    if (str_starts_with($path, 'uploads/')) {
        return app_url('controllers/offer_emploi/' . $path);
    }
...
<head>
    ...
    <link rel="icon" type="image/x-icon" href="<?php echo h(app_url('public/assets/favicon.ico')); ?>" />
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/css/bootstrap.min.css" rel="stylesheet" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <link href="<?php echo h(app_url('public/assets/css/styles.css')); ?>" rel="stylesheet" />
    <style>
        :root {
        ...
        .hero-banner { background: linear-gradient(135deg, rgba(139, 90, 58, 0.72), rgba(46, 107, 62, 0.72)), url('<?php echo h(app_url('public/assets/img/item_pics/IMG_3043.JPG')); ?>'); background-size: cover; background-position: center; color: #F5ECD7; padding: 4rem 0; text-align: center; border-bottom: 5px solid #8B5A3A; margin-bottom: 2rem; }
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto align-items-center gap-2">
                <li class="nav-item"><a class="nav-link" href="<?php echo h(app_url('controllers/home.php')); ?>"><i class="fas fa-home me-1"></i>Accueil</a></li>
                <li class="nav-item"><a class="nav-link" href="<?php echo h(app_url('controllers/projects/projets.php')); ?>"><i class="fas fa-project-diagram me-1"></i>Projets</a></li>
                <li class="nav-item"><a class="nav-link" href="<?php echo h(app_url('controllers/formation/formations.php')); ?>"><i class="fas fa-graduation-cap me-1"></i>Formations</a></li>
                <li class="nav-item"><a class="nav-link" href="<?php echo h(app_url('controllers/invester/invest.php')); ?>"><i class="fas fa-chart-line me-1"></i>Investir</a></li>
                <li class="nav-item"><a class="nav-link active" href="<?php echo h(app_url('controllers/offer_emploi/offres.php')); ?>"><i class="fas fa-briefcase me-1"></i>Emplois</a></li>
                <li class="nav-item"><a class="nav-link" href="<?php echo h(app_url('controllers/user/profile.php')); ?>"><i class="fas fa-user me-1"></i>Profil</a></li>
                <li class="nav-item">
                ...
                <li class="nav-item">
                    <a class="btn-logout" href="<?php echo h(app_url('controllers/session_status.php?action=logout')); ?>">
...
<div class="container mb-5 fade-in">
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                <thead class="table-light">
                    <tr><th>Titre</th><th>Projet</th><th>Budget</th><th>Durée</th><?php if ($hasStatusColumn): ?><th>Statut</th><?php endif; ?><?php if ($hasVerificationColumn): ?><th>Vérification</th><?php endif; ?><th class="text-end">Actions</th></tr>
                </thead>
                <tbody>
        ...
        <div class="offer-create-shell">
            ...
            <div class="body">
                <form method="post" enctype="multipart/form-data" class="row g-4">
                    ...
                    <?php if ($hasStatusColumn): ?>
                    <div class="col-md-3">
                        <label class="form-label fw-bold"><i class="fas fa-toggle-on me-1 text-success"></i>Statut</label>
                        <select class="form-select" name="status">
                            <?php $currentStatus = (string)($editOffer['status'] ?? 'active'); ?>
                            <option value="draft" <?php echo $currentStatus === 'draft' ? 'selected' : ''; ?>>Brouillon</option>
                            <option value="active" <?php echo in_array($currentStatus, ['active', 'actif', 'open'], true) ? 'selected' : ''; ?>>Active</option>
                            <option value="paused" <?php echo $currentStatus === 'paused' ? 'selected' : ''; ?>>En pause</option>
                            <option value="closed" <?php echo in_array($currentStatus, ['closed', 'completed'], true) ? 'selected' : ''; ?>>Clôturée</option>
                        </select>
                    </div>
                    <?php endif; ?>
    ...
    <div class="search-section">
        <form method="get" class="row g-3 align-items-end">
            ...
            <?php if ($hasVerificationColumn): ?>
            <div class="col-md-3">
                <label class="form-label fw-bold"><i class="fas fa-check-double me-1"></i>Vérification</label>
                <select name="verification" class="form-select">
                    <option value="">Toutes</option>
                    <option value="verified" <?php echo $verificationFilter === 'verified' ? 'selected' : ''; ?>>Vérifiées</option>
                    <option value="not_verified" <?php echo $verificationFilter === 'not_verified' ? 'selected' : ''; ?>>Non vérifiées</option>
                </select>
            </div>
            <?php endif; ?>
            <?php if ($hasStatusColumn): ?>
            <div class="col-md-3">
                <label class="form-label fw-bold"><i class="fas fa-signal me-1"></i>Statut offre</label>
                <select name="status" class="form-select">
                    <option value="">Tous</option>
                    <option value="draft" <?php echo $statusFilter === 'draft' ? 'selected' : ''; ?>>Brouillon</option>
                    <option value="active" <?php echo in_array($statusFilter, ['active', 'actif', 'open'], true) ? 'selected' : ''; ?>>Active</option>
                    <option value="paused" <?php echo $statusFilter === 'paused' ? 'selected' : ''; ?>>En pause</option>
                    <option value="closed" <?php echo in_array($statusFilter, ['closed', 'completed'], true) ? 'selected' : ''; ?>>Clôturée</option>
                </select>
            </div>
            <?php endif; ?>
            <div class="col-md-3">
                <label class="form-label fw-bold"><i class="fas fa-sort me-1"></i>Trier par</label>
                <select name="sort" class="form-select">
                    <option value="latest" <?php echo $sortFilter === 'latest' ? 'selected' : ''; ?>>Plus récentes</option>
                    <option value="oldest" <?php echo $sortFilter === 'oldest' ? 'selected' : ''; ?>>Plus anciennes</option>
                    <option value="budget_high" <?php echo $sortFilter === 'budget_high' ? 'selected' : ''; ?>>Budget décroissant</option>
                    <option value="budget_low" <?php echo $sortFilter === 'budget_low' ? 'selected' : ''; ?>>Budget croissant</option>
                    <option value="title_asc" <?php echo $sortFilter === 'title_asc' ? 'selected' : ''; ?>>Titre A→Z</option>
                    <option value="title_desc" <?php echo $sortFilter === 'title_desc' ? 'selected' : ''; ?>>Titre Z→A</option>
                </select>
            </div>
            <div class="col-md-2">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap">
        ...
        <?php if ($search || $competenceFilter || $verificationFilter || $statusFilter || $sortFilter !== 'latest'): ?>
            ...
            <?php foreach ($offers as $offer): ?>
                ...
                <div class="card-offer position-relative">
                    <?php if ($hasImageColumn && !empty($offer['image_path'])): ?>
                        <img class="offer-thumb" src="<?php echo h(normalize_offer_image_url((string)$offer['image_path'])); ?>" alt="<?php echo h($offer['titre']); ?>">
                    <?php else: ?>
                    ...
                    <div class="offer-card-body">
                        ...
                        <?php if ($hasVerificationColumn): ?>
                            <?php
                                $verificationStatus = isset($offer['verification_status']) ? (string)$offer['verification_status'] : 'not_verified';
                            ...
                            ?>
                            <div class="mb-2">
                                <span class="badge <?php echo $verificationClass; ?> px-3 py-2"><?php echo h($verificationLabel); ?></span>
                            </div>
                        <?php endif; ?>
                        ...
                        <div class="mb-3 d-flex flex-wrap gap-2">
                            ...
                            <?php if ($hasStatusColumn): ?>
                                <?php
                                    $cardStatus = (string)($offer['status'] ?? 'active');
                                    $cardStatusClass = in_array($cardStatus, ['active', 'actif', 'open'], true) ? 'bg-success text-white' : (in_array($cardStatus, ['paused'], true) ? 'bg-warning text-dark' : (in_array($cardStatus, ['draft'], true) ? 'bg-secondary text-white' : 'bg-dark text-white'));
                                ?>
                                <span class="offer-chip <?php echo $cardStatusClass; ?>"><i class="fas fa-signal"></i> <?php echo h($cardStatus); ?></span>
                            <?php endif; ?>
        ...
        <nav class="mt-5">
            <ul class="pagination justify-content-center pagination-custom">
                <li class="page-item <?php echo $page <= 1 ? 'disabled' : ''; ?>">
                    <a class="page-link" href="?page=<?php echo $page - 1; ?>&q=<?php echo urlencode($search); ?>&competence=<?php echo urlencode($competenceFilter); ?>&verification=<?php echo urlencode($verificationFilter); ?>&status=<?php echo urlencode($statusFilter); ?>&sort=<?php echo urlencode($sortFilter); ?>"><i class="fas fa-chevron-left"></i></a>
                </li>
                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                    <li class="page-item <?php echo $i === $page ? 'active' : ''; ?>">
                        <a class="page-link" href="?page=<?php echo $i; ?>&q=<?php echo urlencode($search); ?>&competence=<?php echo urlencode($competenceFilter); ?>&verification=<?php echo urlencode($verificationFilter); ?>&status=<?php echo urlencode($statusFilter); ?>&sort=<?php echo urlencode($sortFilter); ?>"><?php echo $i; ?></a>
                    </li>
                <?php endfor; ?>
                <li class="page-item <?php echo $page >= $totalPages ? 'disabled' : ''; ?>">
                    <a class="page-link" href="?page=<?php echo $page + 1; ?>&q=<?php echo urlencode($search); ?>&competence=<?php echo urlencode($competenceFilter); ?>&verification=<?php echo urlencode($verificationFilter); ?>&status=<?php echo urlencode($statusFilter); ?>&sort=<?php echo urlencode($sortFilter); ?>"><i class="fas fa-chevron-right"></i></a>
                </li>
            </ul>
        </nav>
        <?php endif; ?>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-4 mb-4 mb-md-0">
                <img src="<?php echo h(app_url('public/assets/img/logo_herfa.png')); ?>" alt="Logo" height="50" class="mb-3">
            ...
            <div class="col-md-2 mb-4 mb-md-0">
                ...
                <ul class="list-unstyled small">
                    <li class="mb-2"><a href="<?php echo h(app_url('controllers/home.php')); ?>">Accueil</a></li>
                    <li class="mb-2"><a href="<?php echo h(app_url('controllers/projects/projets.php')); ?>">Projets</a></li>
                    <li class="mb-2"><a href="<?php echo h(app_url('controllers/formation/formations.php')); ?>">Formations</a></li>
                    <li class="mb-2"><a href="<?php echo h(app_url('controllers/offer_emploi/offres.php')); ?>">Emplois</a></li>
                </ul>
            </div>
...
<script src="<?php echo h(app_url('public/assets/js/scripts.js')); ?>"></script>
<script>
