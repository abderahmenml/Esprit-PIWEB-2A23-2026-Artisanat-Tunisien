...
class ProfilModel
{
    ...
    /**
     * Cache of resolved table names when aliases/typos are detected.
     * Key: requested table name, Value: actual table name found in DB
     */
    private array $resolvedTables = [];

    /**
     * Known table aliases / common typos to try when a canonical table is missing.
     */
    private array $tableAliases = [
        'profil_professionnel' => ['profil_profetionnel', 'profil_profetionel'],
        'profil_competences'   => ['profil_competence'],
        'profil_competence'    => ['profil_competences'],
    ];

    public function __construct()
    {
        $this->pdo = function_exists('getPDO') ? getPDO() : Database::getPDO();
    }

    /**
     * Retourne la ligne artisan_onboarding pour un utilisateur donné.
     */
    public function getOnboardingByUserId(int $userId): array|null
    {
        try {
            $stmt = $this->pdo->prepare('SELECT * FROM artisan_onboarding WHERE user_id = ? LIMIT 1');
            $stmt->execute([$userId]);
    ...
    public function updateProfil(int $userId, array $data): void
    {
        ...
        foreach (['nom', 'prenom', 'email'] as $column) {
            if ($this->hasColumn('user', $column)) {
                $userSetParts[] = "$column = ?";
                $userParams[] = $data[$column] ?? null;
            }
        }

        if ($this->hasColumn('user', 'telephone')) {
            $userSetParts[] = 'telephone = ?';
            $userParams[] = $data['telephone'] ?? null;
        } elseif ($this->hasColumn('user', 'num_tel')) {
            $userSetParts[] = 'num_tel = ?';
            $userParams[] = $data['telephone'] ?? null;
        }
        ...
        if (!empty($userSetParts)) {
            $userParams[] = $userId;
            $stmt = $this->pdo->prepare(
                'UPDATE user SET ' . implode(', ', $userSetParts) . ' WHERE id_user = ?'
            );
    ...
    public function getPortfolioRealisationsCatalog(): array
    {
        if (!$this->hasTable('portfolio_files') || !$this->hasColumn('portfolio_files', 'realisation')) {
            return [];
        }

        $stmt = $this->pdo->query("
            SELECT DISTINCT realisation
            FROM portfolio_files
            WHERE realisation IS NOT NULL AND TRIM(realisation) <> ''
            ORDER BY realisation ASC
        ");
    ...
    private function hasTable(string $table): bool
    {
        ...
        try {
            ...
            if ($exists) {
                $this->resolvedTables[$table] = $table;
            }
        } catch (Exception) {
            $exists = false;
        }

        // If the exact table wasn't found, try known aliases/typos
        if (!$exists && isset($this->tableAliases[$table])) {
            foreach ($this->tableAliases[$table] as $alias) {
                try {
                    $stmt = $this->pdo->prepare('SHOW TABLES LIKE ?');
                    $stmt->execute([$alias]);
                    if ($stmt->fetchColumn() !== false) {
                        $exists = true;
                        $this->resolvedTables[$table] = $alias;
                        break;
                    }
                } catch (Exception) {
                    // ignore and continue
        ...
        // Also allow reverse mapping: if code asks for an alias, check canonical
        if (!$exists) {
            foreach ($this->tableAliases as $canon => $aliases) {
                if (in_array($table, $aliases, true)) {
                    ...
                    try {
                        ...
                        if ($stmt->fetchColumn() !== false) {
                            $exists = true;
                            $this->resolvedTables[$table] = $canon;
                            break;
                        }
                    } catch (Exception) {
                        // ignore
    ...
    private function hasColumn(string $table, string $column): bool
    {
        ...
        // Use resolved actual table name if one was detected
        $actualTable = $this->resolvedTables[$table] ?? $table;

        try {
            $stmt = $this->pdo->prepare(
                'SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ?'
            );
            $stmt->execute([$actualTable, $column]);
