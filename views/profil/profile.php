...
$profileCssVersion = (string)(@filemtime(dirname(__DIR__, 2) . '/public/assets/css/profile.css') ?: time());
$profileJsVersion = (string)(@filemtime(dirname(__DIR__, 2) . '/public/assets/js/profile.js') ?: time());
$roleLabelMap = [
    'artisan' => 'Artisan',
    'recruteur' => 'Recruteur',
    'admin' => 'Admin',
...
$profileRole = strtolower((string)($user['role'] ?? ($_SESSION['role'] ?? 'artisan')));
$profileRoleLabel = $roleLabelMap[$profileRole] ?? ucfirst($profileRole !== '' ? $profileRole : 'Membre');
$viewedFullName = trim((string)($user['prenom'] ?? '') . ' ' . (string)($user['nom'] ?? ''));
$memberSinceYear = !empty($user['date_creation']) ? date('Y', strtotime((string)$user['date_creation'])) : '';
$heroAvailabilityText = trim((string)($disponibiliteLabel ?? 'Disponible'));
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <link rel="stylesheet" href="<?= htmlspecialchars(app_url('/public/assets/css/profile.css?v=' . $profileCssVersion)) ?>">
    <script src="<?= htmlspecialchars(app_url('/public/assets/js/profile.js?v=' . $profileJsVersion)) ?>" defer></script>
    <style>
        <?= app_header_styles() ?>
        body[data-own-profile="0"] .owner-only { display: none !important; }
        body[data-own-profile="0"] form[action*="controllers/user/profile.php?action="] { display: none !important; }
        .legacy-profile-header { display: none !important; }
    </style>
</head>
<body data-own-profile="<?= $isOwnProfile ? '1' : '0' ?>">
    <?php render_app_header('profile'); ?>
    <!-- HEADER -->
    ...
    <div class="page" id="section-profil">
        ...
        <div class="page-header">
            <h1><?= $isOwnProfile ? 'Mon' : 'Profil de' ?> <span>Profil Professionnel</span></h1>
        ...
        <section class="profile-hero card">
            <div class="profile-hero-copy">
                <span class="profile-hero-kicker"><?= htmlspecialchars($isOwnProfile ? 'Espace professionnel' : 'Profil membre') ?></span>
                <h2><?= htmlspecialchars($viewedFullName !== '' ? $viewedFullName : 'Utilisateur') ?></h2>
                <p>
                    <?= htmlspecialchars($profileRoleLabel) ?>
                    <?php if ($specialite !== ''): ?> · <?= htmlspecialchars($specialite) ?><?php endif; ?>
                    <?php if ($ville !== ''): ?> · <?= htmlspecialchars($ville) ?><?php endif; ?>
                </p>
                <div class="profile-hero-pills">
                    <span class="profile-hero-pill"><i class="fas fa-sparkles"></i> Score profil <?= (int)$profileScore ?>%</span>
                    <span class="profile-hero-pill"><i class="fas fa-signal"></i> Disponibilité <?= htmlspecialchars($heroAvailabilityText) ?></span>
                    <span class="profile-hero-pill"><i class="fas fa-calendar-alt"></i> Membre depuis <?= htmlspecialchars($memberSinceYear !== '' ? $memberSinceYear : 'N/A') ?></span>
                </div>
            </div>
            <div class="profile-hero-metrics">
                <div class="hero-metric-card">
                    <strong><?= (int)$total_projets ?></strong>
                    <span>Projets</span>
                </div>
                <div class="hero-metric-card">
                    <strong><?= htmlspecialchars((string)$note_moyenne) ?></strong>
                    <span>Note moyenne</span>
                </div>
                <div class="hero-metric-card">
                    <strong><?= (int)$suggestedOpportunities ?></strong>
        ...
        <aside class="sidebar">
            ...
            <div class="card profile-card">
                <div class="profile-top">
                    ...
                    ?>
                    <div class="badge-row">
                        <span class="badge"><?= htmlspecialchars($profileRoleLabel) ?></span>
                        <span class="badge <?= htmlspecialchars($disponibiliteBadgeClass) ?>"><?= htmlspecialchars($disponibiliteLabel ?? 'Disponible') ?></span>
                        <?php if ($specialite !== ''): ?>
                            <span class="badge"><?= htmlspecialchars($specialite) ?></span>
                        <?php endif; ?>
                    </div>
                    <button class="edit-btn owner-only" onclick="openModal('edit')">✏️ Modifier mon profil</button>
                </div>
            </div>
            <!-- Statistiques -->
            <div class="card">
            ...
            <div class="card">
                ...
                <div class="availability-row availability-<?= htmlspecialchars($disponibilite ?? 'disponible') ?>" data-availability-visitor="true">
                    ...
                    <div class="avail-sub" data-visitor-hours="true"><?= htmlspecialchars($disponibilite_horaire !== '' ? $disponibilite_horaire : 'Horaires non renseignés') ?></div>
                </div>
            ...
            <div class="card">
                ...
                <div class="contact-row">
                    ...
                    <?= htmlspecialchars($email !== '' ? $email : 'Email non renseigné') ?>
                </div>
                <div class="contact-row">
                    <div class="contact-icon">📞</div>
                    <?= htmlspecialchars((string)($telephone !== '' ? $telephone : 'Téléphone non renseigné'), ENT_QUOTES, 'UTF-8') ?>
                </div>
                ...
                <div class="contact-row">
                    <div class="contact-icon">📍</div>
                    <?= htmlspecialchars($ville !== '' ? $ville : 'Ville non renseignée') ?>
                </div>
            </div>
        </aside>
        <!-- MAIN COL -->
        <main class="main-col">
            ...
            <div class="tab-panel active" id="panel-bio">
                ...
                <?php if (!empty($onboard_skills) || !empty($onboard_craft_focus) || !empty($onboard_interests)): ?>
                    <div class="card">
                        <div class="section-title">Informations Onboarding</div>
                        <?php if (!empty($onboard_craft_focus)): ?>
                            <div style="margin-bottom:.5rem;"><strong>Domaines :</strong>
                                <?php foreach (array_filter(array_map('trim', explode(',', $onboard_craft_focus))) as $chip): ?>
                                    <span class="summary-pill" style="margin-right:.4rem;"><?= htmlspecialchars($chip) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
                        <?php if (!empty($onboard_skills)): ?>
                            <div style="margin-bottom:.5rem;"><strong>Compétences :</strong>
                                <?php foreach (array_filter(array_map('trim', explode(',', $onboard_skills))) as $chip): ?>
                                    <span class="summary-pill" style="margin-right:.4rem;"><?= htmlspecialchars($chip) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
                        <?php if (!empty($onboard_interests)): ?>
                            <div><strong>Intérêts :</strong>
                                <?php foreach (array_filter(array_map('trim', explode(',', $onboard_interests))) as $chip): ?>
                                    <span class="summary-pill" style="margin-right:.4rem;"><?= htmlspecialchars($chip) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem 2rem;">
                        ...
                        <div class="contact-row" style="border-bottom:none;padding:.4rem 0;">
                            ...
                            <div>
                                ...
                                <div style="font-size:.83rem;font-weight:600;color:var(--brun)"><?= htmlspecialchars($memberSinceYear !== '' ? $memberSinceYear : 'N/A') ?></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div id="modal-metier-avance" class="owner-only" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;align-items:center;justify-content:center;">
                ...
                <div class="card">
                    ...
                    <div class="portfolio-grid">
                        <?php if (!empty($portfolioFiles)): ?>
                            ...
                            <?php
                                $portfolioTitle = trim((string)($file['titre'] ?: $file['realisation'] ?: $file['file_name']));
                                $portfolioMeta = trim((string)($file['realisation'] ?? ''));
                                $portfolioIcon = strtoupper(substr($portfolioTitle !== '' ? $portfolioTitle : 'P', 0, 1));
                            ?>
                            <div class="portfolio-item">
                                <div class="portfolio-thumb"><?= htmlspecialchars($portfolioIcon) ?></div>
                                <div class="portfolio-info">
                                    <div class="portfolio-title"><?= htmlspecialchars($portfolioTitle !== '' ? $portfolioTitle : 'Document portfolio') ?></div>
                                    <div class="portfolio-cat"><?= htmlspecialchars($portfolioMeta !== '' ? $portfolioMeta : 'Portfolio PDF') ?></div>
                                </div>
                                <?php if (!empty($file['file_path'])): ?>
                                    <a class="portfolio-overlay" href="<?= htmlspecialchars((string)$file['file_path']) ?>" target="_blank" rel="noopener">Voir le document</a>
                                <?php else: ?>
                                    <div class="portfolio-overlay">Document indisponible</div>
                                <?php endif; ?>
                            </div>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="portfolio-empty-card">
                                <div class="portfolio-empty-icon"><i class="fas fa-folder-open"></i></div>
                                <div class="portfolio-empty-title">Aucun document portfolio</div>
                                <div class="portfolio-empty-text">Ajoutez vos réalisations PDF pour alimenter cette section.</div>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <!-- MODAL MODIFICATION -->
    <div class="modal-overlay owner-only" id="modal-edit" style="display:none">
        <div class="modal">
            ...
            <form action="<?= htmlspecialchars(app_url('/profil/update')) ?>" method="POST" enctype="multipart/form-data">
                ...
                >
                    ...
                    <div class="availability-preview-box">
                        ...
                        <div class="availability-preview-hours" data-availability-preview-hours="true"><?= htmlspecialchars($disponibilite_horaire !== '' ? $disponibilite_horaire : 'Horaires non renseignés') ?></div>
                ...
                <div class="form-group">
                    ...
                    <input type="tel" name="telephone" maxlength="30" pattern="^\+?[0-9][0-9\s().-]{7,19}$" value="<?= htmlspecialchars($telephone ?? '') ?>" placeholder="ex : +216 20 000 000">
                </div>
                <div class="form-group">
                    <label>Ville</label>
