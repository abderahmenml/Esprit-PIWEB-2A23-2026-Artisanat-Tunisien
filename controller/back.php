...
if ($requestMethod === 'POST') {
  ...
  elseif ($action === 'update_quiz') {
    ...
    if ($erreur === '' && !$autoGenerateQuiz) {
      ...
      if ($erreur === '') {
        for ($q = 0; $q < count($quizBuilderQuestionsInput); $q++) {
          ...
          if ($questionText === '' || strlen($questionText) < 3) { $erreur = 'Chaque question du quiz doit contenir au moins 3 caractères.'; break; }
          if ($questionType !== 'choix_unique' && $questionType !== 'choix_multiple' && $questionType !== 'vrai_faux') { $questionType = 'choix_unique'; }
          if ($questionType === 'vrai_faux') {
            $tfCorrectRaw = 'true';
            if (isset($questionInput['tf_correct']) && trim((string) $questionInput['tf_correct']) === 'false') { $tfCorrectRaw = 'false'; }
            $quizQuestionsToInsert[] = ['text' => $questionText, 'type' => 'vrai_faux', 'points' => $questionPoints, 'answers' => [['text' => 'True', 'is_correct' => ($tfCorrectRaw === 'true')], ['text' => 'False', 'is_correct' => ($tfCorrectRaw === 'false')]]];
            continue;
          }
          $rawAnswers = [];
          if (isset($questionInput['answers']) && is_array($questionInput['answers'])) { $rawAnswers = $questionInput['answers']; }
          $cleanAnswers = [];
          $correctCount = 0;
          for ($a = 0; $a < count($rawAnswers); $a++) {
            ...
            $cleanAnswers[] = ['text' => $answerText, 'is_correct' => $isCorrect];
          }
          if (count($cleanAnswers) < 2) { $erreur = 'Chaque question (hors vrai/faux) doit contenir au moins 2 réponses.'; break; }
          if ($correctCount <= 0) { $erreur = 'Chaque question doit avoir au moins une bonne réponse.'; break; }
          if ($questionType === 'choix_unique' && $correctCount !== 1) { $erreur = 'Une question en choix unique doit avoir exactement une seule bonne réponse.'; break; }
          $quizQuestionsToInsert[] = ['text' => $questionText, 'type' => $questionType, 'points' => $questionPoints, 'answers' => $cleanAnswers];
        }
      }
    }
    ...
    if ($erreur === '' && $autoGenerateQuiz) {
      try {
        ...
        if (!$formationRow) { $erreur = 'Formation associée introuvable.'; }
        else {
      ...
      } catch (RuntimeException $e) { $erreur = $e->getMessage(); }
    }
    ...
    if ($erreur === '') {
      try {
        ...
        $quizSetParts = [];
        if ($hasQuizzTitreColumn) { $quizSetParts[] = 'titre = :titre'; }
        if (!$autoGenerateQuiz && $hasQuizzDescriptionColumn) { $quizSetParts[] = 'description = :description'; }
        if ($hasQuizzFormationIdColumn) { $quizSetParts[] = 'formation_id = :formation_id'; }
        if (!$autoGenerateQuiz && $hasQuizzNotePassageColumn) { $quizSetParts[] = 'note_passage = :note_passage'; }
        elseif (!$autoGenerateQuiz && $hasQuizzScoreColumn) { $quizSetParts[] = 'score = :score'; }
        if (!$autoGenerateQuiz && $hasQuizzNbTentativesColumn) { $quizSetParts[] = 'nb_tentatives = :nb_tentatives'; }
        if (!$autoGenerateQuiz && $hasQuizzDureeMinutesColumn) { $quizSetParts[] = 'duree_minutes = :duree_minutes'; }
        elseif (!$autoGenerateQuiz && $hasQuizzDureeColumn) { $quizSetParts[] = 'duree = :duree'; }

        if (count($quizSetParts) > 0) {
          $sqlQuizUpdate = 'UPDATE quizz SET ' . implode(', ', $quizSetParts) . ' WHERE id_quizz = :id_quizz';
          $stmtQuizUpdate = $pdo->prepare($sqlQuizUpdate);
          if ($hasQuizzTitreColumn) { $stmtQuizUpdate->bindValue(':titre', $quizForm['titre']); }
          if (!$autoGenerateQuiz && $hasQuizzDescriptionColumn) {
            $stmtQuizUpdate->bindValue(':description', $quizForm['description'] === '' ? null : $quizForm['description'], $quizForm['description'] === '' ? PDO::PARAM_NULL : PDO::PARAM_STR);
          }
          if ($hasQuizzFormationIdColumn) { $stmtQuizUpdate->bindValue(':formation_id', $quizForm['formation_id'], PDO::PARAM_INT); }
          if (!$autoGenerateQuiz && $hasQuizzNotePassageColumn) { $stmtQuizUpdate->bindValue(':note_passage', (int) $quizForm['note_passage'], PDO::PARAM_INT); }
          elseif (!$autoGenerateQuiz && $hasQuizzScoreColumn) { $stmtQuizUpdate->bindValue(':score', (int) $quizForm['note_passage'], PDO::PARAM_INT); }
          if (!$autoGenerateQuiz && $hasQuizzNbTentativesColumn) { $stmtQuizUpdate->bindValue(':nb_tentatives', (int) $quizForm['nb_tentatives'], PDO::PARAM_INT); }
          if (!$autoGenerateQuiz && $hasQuizzDureeMinutesColumn) { $stmtQuizUpdate->bindValue(':duree_minutes', (int) $quizForm['duree_minutes'], PDO::PARAM_INT); }
          elseif (!$autoGenerateQuiz && $hasQuizzDureeColumn) { $stmtQuizUpdate->bindValue(':duree', (int) $quizForm['duree_minutes'], PDO::PARAM_INT); }
        ...
        $stmtDeleteQuestions = $pdo->prepare('DELETE FROM questions WHERE id_quizz = :id_quizz');
        $stmtDeleteQuestions->bindValue(':id_quizz', $quizForm['id_quizz'], PDO::PARAM_INT);
        $stmtDeleteQuestions->execute();

        $stmtQuestionInsert = $pdo->prepare('INSERT INTO questions (id_quizz, enonce, type, points, ordre) VALUES (:id_quizz, :enonce, :type, :points, :ordre)');
        $stmtAnswerInsert = $pdo->prepare('INSERT INTO reponses (id_question, texte, est_correcte) VALUES (:id_question, :texte, :est_correcte)');
        ...
        for ($q = 0; $q < count($quizQuestionsToInsert); $q++) {
          ...
          $stmtQuestionInsert->bindValue(':ordre', $q + 1, PDO::PARAM_INT);
  ...
  elseif ($action === 'add_quiz') {
    ...
    if ($erreur === '' && $quizForm['formation_id'] <= 0) { $erreur = 'La formation associée est obligatoire.'; }
    if ($erreur === '' && !$quizQuestionsEnabled) { $erreur = 'Les tables questions/reponses sont nécessaires pour ajouter des questions au quiz.'; }

    if ($erreur === '' && !$autoGenerateQuiz) {
      $quizBuilderQuestionsInput = parseQuizBuilderQuestions($_POST);
      if ($quizForm['titre'] === '' || strlen($quizForm['titre']) < 3) { $erreur = 'Le titre doit contenir au moins 3 caractères.'; }
      elseif (!ctype_digit($quizForm['duree_minutes']) || (int) $quizForm['duree_minutes'] <= 0) { $erreur = 'La durée doit être un entier supérieur à 0.'; }
      elseif (!ctype_digit($quizForm['note_passage']) || (int) $quizForm['note_passage'] < 0 || (int) $quizForm['note_passage'] > 100) { $erreur = 'La note de passage doit être un entier entre 0 et 100.'; }
      elseif (!ctype_digit($quizForm['nb_tentatives']) || (int) $quizForm['nb_tentatives'] <= 0) { $erreur = 'Le nombre de tentatives doit être un entier supérieur à 0.'; }
      elseif (count($quizBuilderQuestionsInput) === 0) { $erreur = 'Ajoutez au moins une question au quiz.'; }
      else {
        for ($q = 0; $q < count($quizBuilderQuestionsInput); $q++) {
          $questionInput = $quizBuilderQuestionsInput[$q];
          $questionText = trim((string) $questionInput['text']);
          $questionType = trim((string) $questionInput['type']);
          $questionPoints = 1;
          if (isset($questionInput['points']) && is_numeric($questionInput['points']) && (int) $questionInput['points'] > 0) { $questionPoints = (int) $questionInput['points']; }
          if ($questionText === '' || strlen($questionText) < 3) { $erreur = 'Chaque question du quiz doit contenir au moins 3 caractères.'; break; }
          if ($questionType !== 'choix_unique' && $questionType !== 'choix_multiple' && $questionType !== 'vrai_faux') { $questionType = 'choix_unique'; }
          if ($questionType === 'vrai_faux') {
            $tfCorrectRaw = 'true';
    ...
    if ($erreur === '') {
      try {
        ...
        $quizInsertColumns = [];
        $quizInsertValues = [];
        if ($hasQuizzFormationIdColumn) { $quizInsertColumns[] = 'formation_id'; $quizInsertValues[] = ':formation_id'; }
        if ($hasQuizzDomaineColumn) { $quizInsertColumns[] = 'domaine'; $quizInsertValues[] = ':domaine'; }
        if ($hasQuizzTitreColumn) { $quizInsertColumns[] = 'titre'; $quizInsertValues[] = ':titre'; }
        if ($hasQuizzDescriptionColumn) { $quizInsertColumns[] = 'description'; $quizInsertValues[] = ':description'; }
        if ($hasQuizzQuestionColumn) { $quizInsertColumns[] = 'question'; $quizInsertValues[] = ':question'; }
        if ($hasQuizzNotePassageColumn) { $quizInsertColumns[] = 'note_passage'; $quizInsertValues[] = ':note_passage'; }
        elseif ($hasQuizzScoreColumn) { $quizInsertColumns[] = 'score'; $quizInsertValues[] = ':score'; }
        if ($hasQuizzNbTentativesColumn) { $quizInsertColumns[] = 'nb_tentatives'; $quizInsertValues[] = ':nb_tentatives'; }
        if ($hasQuizzDureeMinutesColumn) { $quizInsertColumns[] = 'duree_minutes'; $quizInsertValues[] = ':duree_minutes'; }
        elseif ($hasQuizzDureeColumn) { $quizInsertColumns[] = 'duree'; $quizInsertValues[] = ':duree'; }

        if (count($quizInsertColumns) === 0) { throw new PDOException('Aucune colonne disponible pour insertion dans quizz.'); }

        $sqlQuizInsert = 'INSERT INTO quizz (' . implode(', ', $quizInsertColumns) . ') VALUES (' . implode(', ', $quizInsertValues) . ')';
        $stmtQuizInsert = $pdo->prepare($sqlQuizInsert);
        if ($hasQuizzFormationIdColumn) { $stmtQuizInsert->bindValue(':formation_id', $quizForm['formation_id'], PDO::PARAM_INT); }
        if ($hasQuizzDomaineColumn) { $stmtQuizInsert->bindValue(':domaine', ''); }
        if ($hasQuizzTitreColumn) { $stmtQuizInsert->bindValue(':titre', $quizForm['titre']); }
        if ($hasQuizzDescriptionColumn) { $stmtQuizInsert->bindValue(':description', $quizForm['description'] === '' ? null : $quizForm['description'], $quizForm['description'] === '' ? PDO::PARAM_NULL : PDO::PARAM_STR); }
        if ($hasQuizzQuestionColumn) { $stmtQuizInsert->bindValue(':question', $quizForm['description'] === '' ? $quizForm['titre'] : $quizForm['description']); }
        if ($hasQuizzNotePassageColumn) { $stmtQuizInsert->bindValue(':note_passage', (int) $quizForm['note_passage'], PDO::PARAM_INT); }
        elseif ($hasQuizzScoreColumn) { $stmtQuizInsert->bindValue(':score', (int) $quizForm['note_passage'], PDO::PARAM_INT); }
        if ($hasQuizzNbTentativesColumn) { $stmtQuizInsert->bindValue(':nb_tentatives', (int) $quizForm['nb_tentatives'], PDO::PARAM_INT); }
        if ($hasQuizzDureeMinutesColumn) { $stmtQuizInsert->bindValue(':duree_minutes', (int) $quizForm['duree_minutes'], PDO::PARAM_INT); }
        elseif ($hasQuizzDureeColumn) { $stmtQuizInsert->bindValue(':duree', (int) $quizForm['duree_minutes'], PDO::PARAM_INT); }
        $stmtQuizInsert->execute();

        $idQuizCree = (int) $pdo->lastInsertId();

        if ($hasQuizzFormationsTable && $idQuizCree > 0) {
          $stmtLienQuizFormation = $pdo->prepare('INSERT INTO quizz_formations (id_formation, id_quizz) VALUES (:id_formation, :id_quizz)');
    ...
    if ($erreur !== '') { $openQuizModal = true; }
  }
  // DELETE FORMATION
  elseif ($action === 'delete_formation') {
    $idFormation = isset($_POST['id_formation']) ? (int) $_POST['id_formation'] : 0;
    if ($idFormation <= 0) { $erreur = 'Identifiant de formation invalide.'; }
    if ($erreur === '') {
      try {
        $stmtDelete = $pdo->prepare('DELETE FROM formations WHERE id_formation = :id');
        $stmtDelete->bindValue(':id', $idFormation, PDO::PARAM_INT);
        $stmtDelete->execute();
        header('Location: back.php?deleted=1');
        exit;
      } catch (PDOException $e) { $erreur = 'Erreur base de données pendant la suppression.'; }
    }
  }
  // DELETE WORKSHOP
  elseif ($action === 'delete_workshop') {
    $idWorkshop = isset($_POST['id_workshop']) ? (int) $_POST['id_workshop'] : 0;
    if ($idWorkshop <= 0) { $erreur = 'Identifiant de workshop invalide.'; }
    if ($erreur === '') {
      try {
        $stmtDelete = $pdo->prepare('DELETE FROM workshops WHERE id_workshop = :id');
  ...
  elseif ($action === 'delete_quiz') {
    ...
    if ($idQuiz <= 0) { $erreur = 'Identifiant de quiz invalide.'; }
    if ($erreur === '') {
      try {
        $stmtDelete = $pdo->prepare('DELETE FROM quizz WHERE id_quizz = :id');
...
if ($afficherFormUpdate === false && isset($_GET['edit_formation']) && is_numeric($_GET['edit_formation'])) {
  ...
  try {
    ...
    if ($formationEdit) {
      ...
      if ($niveauEdit === '') { $niveauEdit = 'debutant'; }
      $afficherFormUpdate = true;
      ...
      if ($updateForm['etat'] !== 'Actif' && $updateForm['etat'] !== 'Inactif' && $updateForm['etat'] !== 'Brouillon') { $updateForm['etat'] = 'Actif'; }
  ...
  } catch (PDOException $e) { $erreur = 'Erreur base de données lors du chargement de la formation.'; }
}
...
if (isset($_GET['edit_quiz']) && is_numeric($_GET['edit_quiz'])) {
  ...
  try {
    ...
    $sql = 'SELECT id_quizz, titre, ' . $editQuizDescCol . ', ' . $editQuizNoteCol . ', ' . $editQuizTentCol . ', ' . $editQuizDureeCol . ', ' . $editQuizFormationCol . ' FROM quizz WHERE id_quizz = :id';
    $stmt = $pdo->prepare($sql);
    ...
    if ($quizData) {
      ...
      if (count($existingQuestions) > 0) {
        for ($qi = 0; $qi < count($existingQuestions); $qi++) {
          ...
          $stmtAnswers = $pdo->prepare('SELECT id_reponse, texte, est_correcte FROM reponses WHERE id_question = :id_question');
          $stmtAnswers->bindValue(':id_question', $qData['id_question'], PDO::PARAM_INT);
          $stmtAnswers->execute();
          $answerRows = $stmtAnswers->fetchAll();
          $answers = [];
          foreach ($answerRows as $ar) {
            $answers[] = ['key' => (string) $ar['id_reponse'], 'text' => trim((string) $ar['texte']), 'is_correct' => (bool) $ar['est_correcte']];
          }
          $oldQuizBuilderQuestions[] = ['key' => (string) $qi, 'text' => trim((string) $qData['enonce']), 'type' => trim((string) $qData['type']), 'points' => (int) $qData['points'], 'tf_correct' => 'true', 'answers' => $answers];
        }
...
if ($niveauFiltre !== '') { $niveauFiltreDb = normalizeNiveau($niveauFiltre); }

$selectQuizNotePassageExpr = 'NULL AS quiz_note_passage';
if (columnExists($pdo, 'quizz', 'note_passage')) { $selectQuizNotePassageExpr = 'qz.note_passage AS quiz_note_passage'; }
elseif (columnExists($pdo, 'quizz', 'score')) { $selectQuizNotePassageExpr = 'qz.score AS quiz_note_passage'; }

$quizLinksParts = [];
if (columnExists($pdo, 'quizz', 'formation_id')) { $quizLinksParts[] = 'SELECT q.formation_id AS id_formation, q.id_quizz FROM quizz q WHERE q.formation_id IS NOT NULL'; }
if (tableExists($pdo, 'quizz_formations')) { $quizLinksParts[] = 'SELECT qf.id_formation, qf.id_quizz FROM quizz_formations qf'; }

$inscritsExpr = '0 AS inscrits';
if (tableExists($pdo, 'inscriptions')) { $inscritsExpr = '(SELECT COUNT(*) FROM inscriptions i WHERE i.id_formation = f.id_formation) AS inscrits'; }
...
if ($hasQuizJoin) {
  $sql = 'SELECT f.id_formation, f.domaine, f.formateur, f.description, f.date_realisation, f.etat, f.duree, f.certification, f.niveau, f.prix,
                 qz.id_quizz AS quiz_id, qz.titre AS quiz_titre, ' . $selectQuizNotePassageExpr . ', ' . $inscritsExpr . '
          FROM formations f
          LEFT JOIN (SELECT liens.id_formation, MIN(liens.id_quizz) AS id_quizz FROM (' . implode(' UNION ALL ', $quizLinksParts) . ') liens GROUP BY liens.id_formation) lq ON lq.id_formation = f.id_formation
          LEFT JOIN quizz qz ON qz.id_quizz = lq.id_quizz';
} else {
  $sql = 'SELECT f.id_formation, f.domaine, f.formateur, f.description, f.date_realisation, f.etat, f.duree, f.certification, f.niveau, f.prix,
                 NULL AS quiz_id, NULL AS quiz_titre, NULL AS quiz_note_passage, ' . $inscritsExpr . '
          FROM formations f';
}
...
$params = [];
if ($recherche !== '') {
  $conditions[] = $hasQuizJoin ? '(f.domaine LIKE :q OR f.formateur LIKE :q OR f.description LIKE :q OR qz.titre LIKE :q)' : '(f.domaine LIKE :q OR f.formateur LIKE :q OR f.description LIKE :q)';
  $params[':q'] = '%' . $recherche . '%';
}
if ($niveauFiltreDb !== '') { $conditions[] = 'f.niveau = :niveau'; $params[':niveau'] = $niveauFiltreDb; }
...
} catch (PDOException $e) { $erreur = 'Erreur lors du chargement des formations.'; }
...
try {
  ...
  if ($hasQuizzNotePassageColumn) { $quizNotePassageExprList = 'q.note_passage'; }
  elseif ($hasQuizzScoreColumn) { $quizNotePassageExprList = 'q.score AS note_passage'; }

  $quizNbTentativesExprList = $hasQuizzNbTentativesColumn ? 'q.nb_tentatives' : 'NULL AS nb_tentatives';
  $quizDureeExprList = $hasQuizzDureeMinutesColumn ? 'q.duree_minutes' : ($hasQuizzDureeColumn ? 'q.duree AS duree_minutes' : 'NULL AS duree_minutes');
  $quizDescriptionExprList = $hasQuizzDescriptionColumn ? 'q.description' : 'NULL AS description';
  $questionCountExprList = $hasQuizQuestionsTable ? '(SELECT COUNT(*) FROM questions qq WHERE qq.id_quizz = q.id_quizz) AS nb_questions' : '0 AS nb_questions';

  $quizListLinkParts = [];
  if ($hasQuizzFormationIdColumn) { $quizListLinkParts[] = 'SELECT q2.id_quizz, q2.formation_id FROM quizz q2 WHERE q2.formation_id IS NOT NULL'; }
  if ($hasQuizzFormationsTable) { $quizListLinkParts[] = 'SELECT qf2.id_quizz, qf2.id_formation AS formation_id FROM quizz_formations qf2'; }

  if (count($quizListLinkParts) > 0) {
    $sqlQuizzes = 'SELECT q.id_quizz, q.titre, ' . $quizDescriptionExprList . ', ' . $quizNotePassageExprList . ', ' . $quizNbTentativesExprList . ', ' . $quizDureeExprList . ', ' . $questionCountExprList . ', lien.formation_id, f.domaine AS formation_titre
                   FROM quizz q
                   LEFT JOIN (SELECT id_quizz, MIN(formation_id) AS formation_id FROM (' . implode(' UNION ALL ', $quizListLinkParts) . ') merged GROUP BY id_quizz) lien ON lien.id_quizz = q.id_quizz
                   LEFT JOIN formations f ON f.id_formation = lien.formation_id
                   ORDER BY q.id_quizz DESC';
  } else {
    $sqlQuizzes = 'SELECT q.id_quizz, q.titre, ' . $quizDescriptionExprList . ', ' . $quizNotePassageExprList . ', ' . $quizNbTentativesExprList . ', ' . $quizDureeExprList . ', ' . $questionCountExprList . ', NULL AS formation_id, NULL AS formation_titre FROM quizz q ORDER BY q.id_quizz DESC';
  }
...
<link href="[fonts.googleapis.com](https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700&family=Calibri:wght@400;600&display=swap)" rel="stylesheet">
...
.simple-actions { display: flex; gap: 10px; align-items: center; flex-wrap: wrap; }
.btn-link { text-decoration: none; }
.form-subtitle { margin: 8px 0; color: var(--marron); font-weight: 700; font-size: 0.9rem; }
.form-note { margin-top: 6px; font-size: 0.78rem; color: var(--gris); }
...
.admin-tab.active { color: var(--marron); border-bottom: 3px solid var(--marron); margin-bottom: -2px; }
.admin-tab:hover { color: var(--brun); }
.tab-content { display: none; }
.tab-content.active { display: block; }
.table-container { overflow-x: auto; }
.workshop-mentor-cell, .quiz-formation-cell { max-width: 150px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

/* ── Quiz builder ─────────────────────────────────────────────────────────── */
.quiz-builder-wrap { display: flex; flex-direction: column; gap: 10px; margin-top: 10px; }
.quiz-builder-question { border: 1px solid rgba(196,154,108,0.35); border-radius: 8px; background: rgba(245,236,215,0.5); padding: 10px; }
.quiz-builder-head { display: flex; justify-content: space-between; align-items: center; gap: 10px; margin-bottom: 8px; }
.quiz-builder-head strong { color: var(--marron); font-size: 0.85rem; }
.quiz-remove-btn { padding: 6px 10px; font-size: 0.75rem; background: #c0392b; color: white; border: none; border-radius: 4px; cursor: pointer; transition: background 0.2s; }
.quiz-remove-btn:hover { background: #a93226; }
.quiz-builder-answer-list { display: flex; flex-direction: column; gap: 8px; margin-top: 6px; }
.quiz-builder-answer-row { display: grid; grid-template-columns: minmax(0,1fr) auto; gap: 8px; align-items: center; }
.quiz-builder-answer-row input[type="text"] { padding: 6px 8px; border: 1px solid rgba(196,154,108,0.5); border-radius: 4px; font-size: 0.85rem; }
.quiz-builder-answer-row label { display: inline-flex; align-items: center; gap: 6px; font-size: 0.78rem; color: var(--gris); white-space: nowrap; }
.quiz-builder-actions { display: flex; justify-content: flex-start; margin-top: 4px; }
.btn-inscrit.vert { background: #27ae60; color: white; padding: 8px 16px; border: none; border-radius: 6px; cursor: pointer; font-size: 0.8rem; transition: background 0.2s; }
.btn-inscrit.vert:hover { background: #219a52; }
.quiz-builder-hidden { display: none; }

/* ── Form grid ────────────────────────────────────────────────────────────── */
.front-form-row-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
.front-form-row-3 { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; }
.modal-footer { display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px; }
.btn-cancel { border: 1px solid rgba(196,154,108,0.5); color: var(--gris); background: transparent; border-radius: 6px; padding: 9px 14px; cursor: pointer; transition: all 0.2s; }
.btn-cancel:hover { background: rgba(196,154,108,0.1); }
.btn-save { border: none; color: var(--creme); background: var(--marron); border-radius: 6px; padding: 9px 14px; cursor: pointer; transition: background 0.2s; }
.btn-save:hover { background: var(--brun); }

@media (max-width: 800px) {
  .front-form-row-2, .front-form-row-3 { grid-template-columns: 1fr; }
...
<div class="main">
  <div class="topbar">
    <div class="breadcrumb">Modules &rsaquo; <strong>BackOffice</strong></div>
  ...
  <div class="content">
      ...
      <form method="post" action="back.php" id="formBackUpdateFormation" novalidate>
        ...
        <div class="front-form-row-2">
          <div class="form-group">
            <label>Titre *</label>
            <input type="text" name="titre" value="<?php echo e($updateForm['titre']); ?>" placeholder="Ex: Initiation à la poterie">
            <span class="field-error" data-err="titre"></span>
          </div>
          <div class="form-group">
            <label>Mentor *</label>
            <input type="text" name="mentor" value="<?php echo e($updateForm['mentor']); ?>" placeholder="Ex: Fatma Ayari">
            <span class="field-error" data-err="mentor"></span>
          </div>
        </div>
        ...
        <div class="front-form-row-3">
          <div class="form-group">
            ...
            <span class="field-error" data-err="niveau"></span>
          </div>
          <div class="form-group">
            <label>Durée (heures) *</label>
            <input type="text" name="duree" value="<?php echo e($updateForm['duree']); ?>" placeholder="Ex: 24">
            <span class="field-error" data-err="duree"></span>
          </div>
          <div class="form-group">
            <label>Prix (TND) *</label>
            <input type="text" name="prix" value="<?php echo e($updateForm['prix']); ?>" placeholder="Ex: 180">
            <span class="field-error" data-err="prix"></span>
          </div>
        </div>
        ...
        <div class="front-form-row-3">
          <div class="form-group">
            ...
            <span class="field-error" data-err="certification"></span>
          </div>
          <div class="form-group">
            <label>Statut</label>
            <select name="etat">
        ...
        <div class="form-group">
          ...
          <span class="field-error" data-err="date_realisation"></span>
        </div>
    ...
    <div id="tab-formations" class="tab-content <?php echo ($typeFiltre === 'formations' ? 'active' : ''); ?>">
      <div class="section-head"><h2>Liste des formations</h2></div>

      <form method="get" action="back.php" class="toolbar" novalidate>
      ...
      <div class="table-container">
        <table>
          <thead>
            <tr>
              <th>ID</th><th>Titre</th><th>Mentor</th><th>Niveau</th><th>Durée</th>
              <th>Inscrits</th><th>Certif.</th><th>Statut</th><th>Prix</th>
              <th>Quiz requis</th><th>Action</th>
            </tr>
          </thead>
          <tbody>
            <?php if (count($formations) === 0): ?>
              <tr><td colspan="11" style="text-align:center;padding:20px;">Aucune formation trouvée.</td></tr>
            <?php else: ?>
              ...
              <tr>
                ...
                <td><span class="badge <?php echo e(niveauBadgeClass($f['niveau'])); ?>"><?php echo e(niveauLabel($f['niveau'])); ?></span></td>
                <td><?php echo e((string) $f['duree']); ?>h</td>
                <td><?php echo e((string) $f['inscrits']); ?></td>
                <td><?php echo certifOui($f['certification']) ? '✅' : '—'; ?></td>
                <td><span class="badge <?php echo e(statutBadgeClass($f['etat'])); ?>"><?php echo e((string) $f['etat']); ?></span></td>
    ...
    <div id="tab-workshops" class="tab-content <?php echo ($typeFiltre === 'workshops' ? 'active' : ''); ?>">
      <div class="section-head"><h2>Liste des workshops</h2></div>
      <div class="table-container">
        <table>
          <thead>
            <tr>
              <th>ID</th><th>Titre</th><th>Description</th><th>Mentor</th>
              <th>Formation associée</th><th>Durée</th><th>Date</th><th>Lieu</th>
              <th>Places</th><th>Prix</th><th>Certif.</th><th>Statut</th><th>Action</th>
            </tr>
          </thead>
          <tbody>
            <?php if (count($workshops) === 0): ?>
              <tr><td colspan="13" style="text-align:center;padding:20px;">Aucun workshop trouvé.</td></tr>
            <?php else: ?>
              ...
              <tr>
                ...
                <td><span class="badge <?php echo e(statutBadgeClass($w['statut'])); ?>"><?php echo e($w['statut']); ?></span></td>
                <td>
                  <div class="action-cell">
                    <form method="get" action="back.php">
    ...
    <div id="tab-quizzes" class="tab-content <?php echo ($typeFiltre === 'quizzes' ? 'active' : ''); ?>">
      <div class="section-head"><h2>Liste des quiz</h2></div>
      <div class="table-container">
        <table>
          <thead>
            <tr>
              <th>ID</th><th>Titre</th><th>Description</th><th>Formation associée</th>
              <th>Note de passage</th><th>Tentatives</th><th>Durée</th><th>Questions</th><th>Action</th>
            </tr>
          </thead>
          <tbody>
            <?php if (count($quizzes) === 0): ?>
              <tr><td colspan="9" style="text-align:center;padding:20px;">Aucun quiz trouvé.</td></tr>
            <?php else: ?>
              ...
              <tr>
                ...
                <td class="workshop-mentor-cell" title="<?php echo e((string) $quizItem['description']); ?>">
                  <?php $qd = trim((string) $quizItem['description']); echo $qd !== '' ? e(mb_substr($qd, 0, 55)) . (mb_strlen($qd) > 55 ? '…' : '') : '<span style="color:var(--gris);">—</span>'; ?>
                </td>
                <td class="quiz-formation-cell">
                  <?php if (trim((string) $quizItem['formation_titre']) !== ''): ?>
                    <strong><?php echo e($quizItem['formation_titre']); ?></strong><br>
                    <span style="color:var(--gris);font-size:0.75rem;">ID: <?php echo e((string) $quizItem['formation_id']); ?></span>
                ...
                <td>
                  <?php if (is_numeric($quizItem['note_passage'])): ?>
                    <span class="badge badge-actif"><?php echo e((string)(int)$quizItem['note_passage']); ?>%</span>
                  <?php else: ?><span style="color:var(--gris);">—</span><?php endif; ?>
                </td>
                <td><?php $nt = $quizItem['nb_tentatives']; echo is_numeric($nt) ? e((string)(int)$nt) : '<span style="color:var(--gris);">—</span>'; ?></td>
                <td><?php $dm = $quizItem['duree_minutes']; echo is_numeric($dm) && (int)$dm > 0 ? e((string)(int)$dm).' min' : '<span style="color:var(--gris);">—</span>'; ?></td>
                <td>
                  <?php $nbQ = (int)($quizItem['nb_questions'] ?? 0); ?>
                  <?php if ($nbQ > 0): ?>
                    <span class="badge badge-inter"><?php echo e((string)$nbQ); ?> question<?php echo $nbQ > 1 ? 's' : ''; ?></span>
                  <?php else: ?><span style="color:var(--gris);">Aucune</span><?php endif; ?>
                </td>
                <td>
                  <div class="action-cell">
                    <form method="get" action="back.php">
...
════════════════════════════════════════════════════════════════════════════ -->
<div class="modal-overlay<?php echo ($openFormationModal ? ' open' : ''); ?>"
     id="modalAjoutFormation"
     onclick="fermerModalSiExterieur(event,'modalAjoutFormation')">
  <div class="modal">
    <div class="modal-header">
      <div><h2>Ajouter une formation</h2><p>Les données seront enregistrées dans la base</p></div>
    ...
    <div class="modal-body">
      <form method="post" action="back.php" id="formAjoutFormation" novalidate>
        ...
        <div class="front-form-row-2">
          <div class="form-group">
            <label>Titre *</label>
            <input type="text" name="titre" id="af_titre" placeholder="Ex: Initiation à la poterie">
            <span class="field-error" id="af_titre_err"></span>
          </div>
          <div class="form-group">
            <label>Mentor *</label>
            <input type="text" name="mentor" id="af_mentor" placeholder="Ex: Fatma Ayari">
            <span class="field-error" id="af_mentor_err"></span>
          </div>
        </div>

        <div class="form-group">
          <label>Description</label>
          <textarea name="description" id="af_description" rows="3" placeholder="Description de la formation"></textarea>
          <span class="field-error" id="af_description_err"></span>
        </div>

        <div class="front-form-row-3">
          <div class="form-group">
            <label>Niveau *</label>
            <select name="niveau" id="af_niveau">
              <option value="debutant">Débutant</option>
              <option value="intermediaire">Intermédiaire</option>
              <option value="avance">Avancé</option>
            </select>
            <span class="field-error" id="af_niveau_err"></span>
          </div>
          <div class="form-group">
            <label>Durée (heures) *</label>
            <input type="text" name="duree" id="af_duree" placeholder="Ex: 24">
            <span class="field-error" id="af_duree_err"></span>
          </div>
          <div class="form-group">
            <label>Prix (TND) *</label>
            <input type="text" name="prix" id="af_prix" placeholder="Ex: 180">
            <span class="field-error" id="af_prix_err"></span>
          </div>
        </div>
        ...
        <div class="front-form-row-3">
          <div class="form-group">
            <label>Certification</label>
            <select name="certification" id="af_certification">
              <option value="oui">Oui</option>
              <option value="non">Non</option>
            </select>
            <span class="field-error" id="af_certification_err"></span>
          </div>
          <div class="form-group">
            <label>Statut</label>
            <select name="etat" id="af_etat">
              <option value="Actif">Actif</option>
              <option value="Inactif">Inactif</option>
              <option value="Brouillon">Brouillon</option>
            </select>
            <span class="field-error" id="af_etat_err"></span>
          </div>
          <div class="form-group">
            <label>Date de réalisation</label>
            <input type="date" name="date_realisation" id="af_date_realisation">
            <span class="field-error" id="af_date_realisation_err"></span>
          </div>
        </div>

        <div class="form-group">
          <label>Lien vidéo (optionnel)</label>
          <input type="url" name="lien_video" id="af_lien_video" placeholder="https://...">
          <span class="field-error" id="af_lien_video_err"></span>
        </div>
...
<!-- ═══════════════════════════════════════════════════════════════════════════
     MODAL — AJOUTER / MODIFIER UN WORKSHOP
════════════════════════════════════════════════════════════════════════════ -->
<div class="modal-overlay<?php echo ($openWorkshopModal ? ' open' : ''); ?>"
     id="modalAjoutWorkshop"
     onclick="fermerModalSiExterieur(event,'modalAjoutWorkshop')">
  <div class="modal">
    <div class="modal-header">
      <div>
    ...
    <div class="modal-body">
      <form method="post" action="back.php" id="formAjoutWorkshop" novalidate>
        ...
        <div class="form-group">
          ...
          <input type="text" name="titre" id="aw_titre" value="<?php echo e($workshopForm['titre']); ?>" placeholder="Ex: Atelier pratique de poterie">
          <span class="field-error" id="aw_titre_err"></span>
        </div>

        <div class="form-group">
          <label>Mentor ID (optionnel)</label>
          <input type="number" min="1" step="1" name="mentor_id" id="aw_mentor_id" value="<?php echo e($workshopForm['mentor_id']); ?>" placeholder="Ex: 3">
          <span class="field-error" id="aw_mentor_id_err"></span>
        </div>

        <div class="form-group">
          <label>Formation associée *</label>
          <select name="formation_id" id="aw_formation_id">
            <option value="">-- Sélectionnez une formation --</option>
            <?php foreach ($formations as $f): ?>
              <option value="<?php echo e((string) $f['id_formation']); ?>"
                <?php echo ($workshopForm['formation_id'] == $f['id_formation'] ? 'selected' : ''); ?>>
                <?php echo e($f['domaine']); ?> (ID: <?php echo e((string) $f['id_formation']); ?>)
              </option>
            <?php endforeach; ?>
          </select>
          <span class="field-error" id="aw_formation_id_err"></span>
        </div>

        <div class="form-group">
          <label>Description workshop *</label>
          <textarea name="description" id="aw_description" rows="3" placeholder="Description du workshop"><?php echo e($workshopForm['description']); ?></textarea>
          <span class="field-error" id="aw_description_err"></span>
        </div>

        <div class="front-form-row-3">
          <div class="form-group">
            <label>Durée (heures) *</label>
            <input type="number" min="1" step="1" name="duree" id="aw_duree" value="<?php echo e($workshopForm['duree']); ?>" placeholder="Ex: 2">
            <span class="field-error" id="aw_duree_err"></span>
          </div>
          <div class="form-group">
            <label>Date atelier (optionnel)</label>
            <input type="datetime-local" name="date_atelier" id="aw_date_atelier" value="<?php echo e($workshopForm['date_atelier']); ?>">
            <span class="field-error" id="aw_date_atelier_err"></span>
          </div>
          <div class="form-group">
            <label>Lieu *</label>
            <input type="text" name="lieu" id="aw_lieu" value="<?php echo e($workshopForm['lieu']); ?>" placeholder="Ex: Tunis">
            <span class="field-error" id="aw_lieu_err"></span>
          </div>
        </div>
        ...
        <div class="front-form-row-3">
          <div class="form-group">
            ...
            <input type="number" min="1" step="1" name="places_max" id="aw_places_max" value="<?php echo e($workshopForm['places_max']); ?>" placeholder="Ex: 20">
            <span class="field-error" id="aw_places_max_err"></span>
          </div>
          <div class="form-group">
            <label>Prix (TND) *</label>
            <input type="number" min="0" step="0.01" name="prix" id="aw_prix" value="<?php echo e($workshopForm['prix']); ?>" placeholder="Ex: 120">
            <span class="field-error" id="aw_prix_err"></span>
          </div>
          <div class="form-group">
            <label>Certification</label>
            <select name="certification" id="aw_certification">
            ...
            <span class="field-error" id="aw_certification_err"></span>
          </div>
        </div>

        <div class="form-group">
          <label>Statut workshop</label>
          <select name="statut" id="aw_statut">
            <option value="a_venir"  <?php echo ($workshopForm['statut'] === 'a_venir'  ? 'selected' : ''); ?>>À venir</option>
            <option value="en_cours" <?php echo ($workshopForm['statut'] === 'en_cours' ? 'selected' : ''); ?>>En cours</option>
            <option value="termine"  <?php echo ($workshopForm['statut'] === 'termine'  ? 'selected' : ''); ?>>Terminé</option>
            <option value="annule"   <?php echo ($workshopForm['statut'] === 'annule'   ? 'selected' : ''); ?>>Annulé</option>
          </select>
          <span class="field-error" id="aw_statut_err"></span>
        </div>
...
<!-- ═══════════════════════════════════════════════════════════════════════════
     MODAL — AJOUTER / MODIFIER UN QUIZ
════════════════════════════════════════════════════════════════════════════ -->
<div class="modal-overlay<?php echo ($openQuizModal ? ' open' : ''); ?>"
     id="modalAjoutQuiz"
     onclick="fermerModalSiExterieur(event,'modalAjoutQuiz')">
  <div class="modal">
    <div class="modal-header">
      <div>
    ...
    <div class="modal-body">
      <form method="post" action="back.php" id="formAjoutQuiz" novalidate>
        ...
        <div class="form-group">
          <label>Formation associée *</label>
          <select name="formation_id" id="aqz_formation_id">
            <option value="">-- Sélectionnez une formation --</option>
            <?php foreach ($formations as $f): ?>
              <option value="<?php echo e((string) $f['id_formation']); ?>"
                <?php echo ($quizForm['formation_id'] == $f['id_formation'] ? 'selected' : ''); ?>>
                <?php echo e($f['domaine']); ?> (ID: <?php echo e((string) $f['id_formation']); ?>)
              </option>
            <?php endforeach; ?>
          </select>
          <span class="field-error" id="aqz_formation_id_err"></span>
        </div>
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn-save"><?php echo ($editQuizId > 0 ? 'Régénérer' : 'Générer'); ?></button>
        </div>
      </form>
    </div>
  </div>
...
document.querySelectorAll('.admin-tab').forEach(function(tab) {
  tab.addEventListener('click', function() {
    ...
    ['edit_formation','edit_workshop','edit_quiz'].forEach(function(k){ url.searchParams.delete(k); });
...
function fermerModal(id) {
  ...
  var url = new URL(window.location.href);
  ['open_add_formation','open_add_workshop','open_add_quiz',
   'edit_formation','edit_workshop','edit_quiz'].forEach(function(k){ url.searchParams.delete(k); });
  window.history.replaceState({}, '', url);
}
function fermerModalSiExterieur(e, id) {
  if (e.target && e.target.classList.contains('modal-overlay')) fermerModal(id);
}

// ── Field-error helpers ───────────────────────────────────────────────────────
/**
 * Show or clear a field error.
 * errId  – id of the <span class="field-error"> element
 * msg    – non-empty string to show, or '' / falsy to clear
 */
function fieldErr(errId, msg) {
  ...
  if (msg) {
    ...
    var inp = span.previousElementSibling;
...
/**
 * Clear errors for a set of fields.
 * prefix  – e.g. 'af_'
 * names   – array of field name strings (without prefix / _err suffix)
 */
function clearErrs(prefix, names) {
  names.forEach(function(n){ fieldErr(prefix + n + '_err', ''); });
}

/**
 * Attach "clear on change/input" listeners.
 */
function attachClear(prefix, names) {
  names.forEach(function(n) {
    var el = document.getElementById(prefix + n);
    if (!el) return;
    var evt = (el.tagName === 'SELECT' || el.tagName === 'TEXTAREA') ? 'change' : 'input';
    el.addEventListener(evt, function(){ fieldErr(prefix + n + '_err', ''); });
  });
}

// ── Validate: Add Formation ───────────────────────────────────────────────────
var AF = ['titre','mentor','description','niveau','duree','prix',
          'certification','etat','date_realisation','lien_video'];

function validateAddFormation() {
  clearErrs('af_', AF);
  var ok = true;

  function v(id, msg){ fieldErr('af_' + id + '_err', msg); ok = false; }

  var titre = (document.getElementById('af_titre')||{value:''}).value.trim();
  if (titre.length < 3) v('titre', 'Le titre doit contenir au moins 3 caractères.');

  var mentor = (document.getElementById('af_mentor')||{value:''}).value.trim();
  if (mentor === '') v('mentor', 'Le mentor est obligatoire.');

  var niveau = (document.getElementById('af_niveau')||{value:''}).value;
  if (!['debutant','intermediaire','avance'].includes(niveau)) v('niveau', 'Niveau invalide.');

  var duree = (document.getElementById('af_duree')||{value:''}).value.trim();
  if (!/^\d+$/.test(duree) || parseInt(duree, 10) <= 0) v('duree', 'La durée doit être un entier > 0.');

  var prix = (document.getElementById('af_prix')||{value:''}).value.trim().replace(',','.');
  if (prix === '' || isNaN(parseFloat(prix))) v('prix', 'Le prix doit être un nombre valide.');

  var certif = (document.getElementById('af_certification')||{value:''}).value;
  if (!['oui','non'].includes(certif)) v('certification', 'Certification invalide.');

  var etat = (document.getElementById('af_etat')||{value:''}).value;
  if (!['Actif','Inactif','Brouillon'].includes(etat)) v('etat', 'Statut invalide.');

  var date = (document.getElementById('af_date_realisation')||{value:''}).value.trim();
  if (date !== '' && isNaN(Date.parse(date))) v('date_realisation', 'Date invalide.');

  var lien = (document.getElementById('af_lien_video')||{value:''}).value.trim();
  if (lien !== '' && !/^https?:\/\/.+/.test(lien)) v('lien_video', 'Le lien doit commencer par http:// ou https://.');

  return ok;
}

// ── Validate: Add / Update Workshop ──────────────────────────────────────────
var AW = ['titre','mentor_id','formation_id','description','duree',
          'date_atelier','lieu','places_max','prix','certification','statut'];

function validateAddWorkshop() {
  clearErrs('aw_', AW);
  var ok = true;

  function v(id, msg){ fieldErr('aw_' + id + '_err', msg); ok = false; }

  var titre = (document.getElementById('aw_titre')||{value:''}).value.trim();
  if (titre.length < 3) v('titre', 'Le titre doit contenir au moins 3 caractères.');

  var mentorId = (document.getElementById('aw_mentor_id')||{value:''}).value.trim();
  if (mentorId !== '' && (!/^\d+$/.test(mentorId) || parseInt(mentorId,10) <= 0))
    v('mentor_id', 'Le mentor ID doit être un entier positif.');

  var formId = (document.getElementById('aw_formation_id')||{value:''}).value;
  if (!formId || parseInt(formId, 10) <= 0) v('formation_id', 'Veuillez sélectionner une formation.');

  var desc = (document.getElementById('aw_description')||{value:''}).value.trim();
  if (desc.length < 10) v('description', 'La description doit contenir au moins 10 caractères.');

  var duree = String((document.getElementById('aw_duree')||{value:''}).value).trim();
  if (!/^\d+$/.test(duree) || parseInt(duree, 10) <= 0) v('duree', 'La durée doit être un entier > 0.');

  var lieu = (document.getElementById('aw_lieu')||{value:''}).value.trim();
  if (lieu.length < 2) v('lieu', 'Le lieu est obligatoire (min. 2 caractères).');

  var places = String((document.getElementById('aw_places_max')||{value:''}).value).trim();
  if (!/^\d+$/.test(places) || parseInt(places, 10) <= 0) v('places_max', 'Le nombre de places doit être un entier > 0.');

  var prix = String((document.getElementById('aw_prix')||{value:''}).value).trim().replace(',','.');
  var prixNum = parseFloat(prix);
  if (prix === '' || isNaN(prixNum)) v('prix', 'Le prix doit être un nombre valide.');
  else if (prixNum < 0) v('prix', 'Le prix doit être ≥ 0.');

  var certif = (document.getElementById('aw_certification')||{value:''}).value;
  if (!['oui','non'].includes(certif)) v('certification', 'Certification invalide.');

  var statut = (document.getElementById('aw_statut')||{value:''}).value;
  if (!['a_venir','en_cours','termine','annule'].includes(statut)) v('statut', 'Statut invalide.');

  return ok;
}

// ── Validate: Add Quiz (formation required) ───────────────────────────────────
function validateAddQuiz() {
  fieldErr('aqz_formation_id_err', '');
  var formId = (document.getElementById('aqz_formation_id')||{value:''}).value;
  if (!formId || parseInt(formId, 10) <= 0) {
    fieldErr('aqz_formation_id_err', 'Veuillez sélectionner une formation.');
    return false;
  }
  return true;
}

// ── Validate: Update Formation (inline panel) ─────────────────────────────────
function validateUpdateFormation(form) {
  // Clear all
  form.querySelectorAll('[data-err]').forEach(function(span){
    span.textContent = ''; span.classList.remove('visible');
    var inp = span.previousElementSibling;
    if (inp) inp.classList.remove('input-error');
  });
  ...
  var titre = (form.querySelector('[name="titre"]')||{value:''}).value.trim();
  if (titre.length < 3) v('titre', 'Le titre doit contenir au moins 3 caractères.');

  var mentor = (form.querySelector('[name="mentor"]')||{value:''}).value.trim();
  if (mentor === '') v('mentor', 'Le mentor est obligatoire.');

  var niveau = (form.querySelector('[name="niveau"]')||{value:''}).value;
  if (!['debutant','intermediaire','avance'].includes(niveau)) v('niveau', 'Niveau invalide.');

  var duree = (form.querySelector('[name="duree"]')||{value:''}).value.trim();
  if (!/^\d+$/.test(duree) || parseInt(duree,10) <= 0) v('duree', 'La durée doit être un entier > 0.');

  var prix = (form.querySelector('[name="prix"]')||{value:''}).value.trim().replace(',','.');
  if (prix === '' || isNaN(parseFloat(prix))) v('prix', 'Le prix doit être un nombre valide.');

  var certif = (form.querySelector('[name="certification"]')||{value:''}).value;
  if (!['oui','non'].includes(certif)) v('certification', 'Certification invalide.');

  var etat = (form.querySelector('[name="etat"]')||{value:''}).value;
  if (!['Actif','Inactif','Brouillon'].includes(etat)) v('etat', 'Statut invalide.');

  var date = (form.querySelector('[name="date_realisation"]')||{value:''}).value.trim();
  if (date !== '' && isNaN(Date.parse(date))) v('date_realisation', 'Date invalide.');

  var lien = (form.querySelector('[name="lien_video"]')||{value:''}).value.trim();
  if (lien !== '' && !/^https?:\/\/.+/.test(lien)) v('lien_video', 'Le lien doit commencer par http:// ou https://.');
...
function qbAnswerRow(qi, ai, text, correct) {
  return '<div class="quiz-builder-answer-row">'
    + '<input type="text" name="quiz_answer_text['+qi+']['+ai+']" value="'+qbEsc(text)+'" placeholder="Texte de la réponse">'
    + '<label><input type="checkbox" name="quiz_answer_correct['+qi+'][]" value="'+ai+'"'+(correct?' checked':'')+'> Bonne réponse</label>'
    + '</div>';
}
function qbRefreshLabels() {
  document.querySelectorAll('#quizBuilderQuestions .quiz-builder-question').forEach(function(c,i){
    var lbl = c.querySelector('.quiz-builder-number');
    if (lbl) lbl.textContent = String(i+1);
  });
}
function onQuizBuilderTypeChange(sel, qi) {
  var tf  = document.getElementById('quizTrueFalseBox'+qi);
  var ans = document.getElementById('quizAnswersBox'+qi);
  if (!tf||!ans) return;
  if (sel.value === 'vrai_faux') { tf.classList.remove('quiz-builder-hidden'); ans.classList.add('quiz-builder-hidden'); }
  else { tf.classList.add('quiz-builder-hidden'); ans.classList.remove('quiz-builder-hidden'); }
}
function addQuizBuilderAnswer(qi) {
  var card = document.querySelector('.quiz-builder-question[data-question-index="'+qi+'"]');
  var list = document.getElementById('quizAnswerList'+qi);
  if (!card||!list) return;
  var next = parseInt(card.getAttribute('data-next-answer-index'),10)||list.children.length;
  list.insertAdjacentHTML('beforeend', qbAnswerRow(qi, next, '', false));
  card.setAttribute('data-next-answer-index', String(next+1));
}
function removeQuizBuilderQuestion(btn) {
  var card = btn.closest('.quiz-builder-question');
  if (!card) return;
  card.remove();
  qbRefreshLabels();
  var c = document.getElementById('quizBuilderQuestions');
  if (c && c.querySelectorAll('.quiz-builder-question').length === 0) addQuizBuilderQuestion();
}
function addQuizBuilderQuestion() {
  var c = document.getElementById('quizBuilderQuestions');
  if (!c) return;
  var next = parseInt(c.getAttribute('data-next-question-index'),10)
             || c.querySelectorAll('.quiz-builder-question').length;
  var qi = String(next);
  c.setAttribute('data-next-question-index', String(next+1));
  var html = '<div class="quiz-builder-question" data-question-index="'+qi+'" data-next-answer-index="2">'
    + '<div class="quiz-builder-head"><strong>Question <span class="quiz-builder-number">0</span></strong>'
    + '<button type="button" class="quiz-remove-btn" onclick="removeQuizBuilderQuestion(this)">Supprimer</button></div>'
    + '<div class="form-group"><label>Texte de la question *</label>'
    + '<input type="text" name="quiz_question_text['+qi+']" placeholder="Ex: Quelle matière est utilisée en vannerie ?"></div>'
    + '<div class="front-form-row-3">'
    + '<div class="form-group"><label>Type</label>'
    + '<select class="quiz-builder-type-select" data-question-index="'+qi+'" name="quiz_question_type['+qi+']" onchange="onQuizBuilderTypeChange(this,\''+qi+'\')">'
    + '<option value="choix_unique">Choix unique</option>'
    + '<option value="choix_multiple">Choix multiple</option>'
    + '<option value="vrai_faux">Vrai / Faux</option></select></div>'
    + '<div class="form-group"><label>Points</label>'
    + '<input type="number" min="1" step="1" name="quiz_question_points['+qi+']" value="1"></div>'
    + '<div class="form-group quiz-builder-hidden" id="quizTrueFalseBox'+qi+'">'
    + '<label>Bonne réponse</label>'
    + '<select name="quiz_tf_correct['+qi+']"><option value="true">True</option><option value="false">False</option></select></div></div>'
    + '<div class="form-group" id="quizAnswersBox'+qi+'"><label>Réponses *</label>'
    + '<div class="quiz-builder-answer-list" id="quizAnswerList'+qi+'">'
    + qbAnswerRow(qi,0,'',false)+qbAnswerRow(qi,1,'',false)+'</div>'
    + '<button type="button" class="btn-inscrit vert" onclick="addQuizBuilderAnswer(\''+qi+'\')">+ Ajouter une réponse</button>'
    + '</div></div>';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  /* --- Add Formation -------------------------------------------------------- */
  var formAF = document.getElementById('formAjoutFormation');
  if (formAF) {
    formAF.addEventListener('submit', function(e){ if (!validateAddFormation()) e.preventDefault(); });
    attachClear('af_', AF);
    // also clear on the select elements that don't fire 'input'
    ['af_niveau','af_certification','af_etat'].forEach(function(id){
      var el = document.getElementById(id);
      if (el) el.addEventListener('change', function(){ fieldErr(id.replace('af_','af_')+'_err',''); });
    });
  }

  /* --- Add / Update Workshop ------------------------------------------------ */
  var formAW = document.getElementById('formAjoutWorkshop');
  if (formAW) {
    formAW.addEventListener('submit', function(e){ if (!validateAddWorkshop()) e.preventDefault(); });
    attachClear('aw_', AW);
  }

  /* --- Add / Regenerate Quiz ------------------------------------------------ */
  var formAQZ = document.getElementById('formAjoutQuiz');
  if (formAQZ) {
    formAQZ.addEventListener('submit', function(e){ if (!validateAddQuiz()) e.preventDefault(); });
    var aqzSel = document.getElementById('aqz_formation_id');
    if (aqzSel) aqzSel.addEventListener('change', function(){ fieldErr('aqz_formation_id_err',''); });
  }

  /* --- Update Formation (inline panel) -------------------------------------- */
  var formUF = document.getElementById('formBackUpdateFormation');
  if (formUF) {
    formUF.addEventListener('submit', function(e){ if (!validateUpdateFormation(formUF)) e.preventDefault(); });
    formUF.querySelectorAll('input, textarea, select').forEach(function(el){
      var evt = el.tagName === 'SELECT' ? 'change' : 'input';
      el.addEventListener(evt, function(){
        el.classList.remove('input-error');
        var name = el.getAttribute('name');
        var span = formUF.querySelector('[data-err="'+name+'"]');
  ...
  /* --- Quiz builder init ---------------------------------------------------- */
  document.querySelectorAll('.quiz-builder-type-select').forEach(function(sel){
    var qi = sel.getAttribute('data-question-index');
    if (qi) onQuizBuilderTypeChange(sel, qi);
  });
  qbRefreshLabels();
});
</script>
